...
if __name__ == '__main__':
    ...
    if args.dataset == 'shortestpath':
        ...
        features, costs = pyepo.data.shortestpath.genData(args.n_samples, args.n_features, grid, args.degree, args.noise_width)
        model = pyepo.model.grb.shortestPathModel(grid)

        # build dataset
        dataset = pyepo.data.dataset.optDataset(model, features, costs)

    elif args.dataset == 'portfolio':
        num_assets = 50 # number of assets

        covariance, features, costs = pyepo.data.portfolio.genData(args.n_samples, args.n_features, num_assets, deg=4, noise_level=1, seed=135)
        model = pyepo.model.grb.portfolioModel(num_assets, covariance)
    ...
    model = CVAE(context.size(-1), obs.size(-1), args.latent_dim, args.latent_dist, args.samples_per_latent)
    model.to(device)
