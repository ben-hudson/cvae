...
class CVAE(nn.Module):
    def __init__(self, context_dim: int, obs_dim: int, latent_dim: int, latent_dist: str = "normal", samples_per_latent: int=1) -> None:
    ...
    def _sample_latents(self, dist):
        latents = dist.rsample((self.samples_per_latent, )) # returns shape (samples_per_latent, batch_size, latent_dim)
        latents = latents.permute(1, 2, 0).flatten(1) # reshape to (batch_size, latent_dim*samples_per_latent)
        return latents
    ...
    def forward(self, context: torch.Tensor, obs: torch.Tensor):
        ...
        latents_hat = self._sample_latents(posterior)
    ...
    def sample(self, context: torch.Tensor):
        ...
        latents_hat = self._sample_latents(prior)
        obs_hat = self._generate_obs(context, latents_hat)
