...
class PortfolioDataset(pyepo.data.dataset.optDataset):
    ...
    def norm(self, feats=None, costs=None, sols=None, objs=None):
        ...
        feats_normed = (feats - self.means.feats.to(feats.device))/self.scales.feats.to(feats.device) if feats is not None else None
        costs_normed = (costs - self.means.costs.to(costs.device))/self.scales.costs.to(costs.device) if costs is not None else None
        sols_normed = (sols - self.means.sol.to(sols.device))/self.scales.sol.to(sols.device) if sols is not None else None
        objs_normed = (objs - self.means.obj.to(objs.device))/self.scales.obj.to(objs.device) if objs is not None else None
    ...
    def unnorm(self, feats_normed=None, costs_normed=None, sols_normed=None, objs_normed=None):
        ...
        feats = self.means.feats.to(feats_normed.device) + self.scales.feats.to(feats_normed.device)*feats_normed if feats_normed is not None else None
        costs = self.means.costs.to(costs_normed.device) + self.scales.costs.to(costs_normed.device)*costs_normed if costs_normed is not None else None
        sols = self.means.sol.to(sols_normed.device) + self.scales.sol.to(sols_normed.device)*sols_normed if sols_normed is not None else None
        objs = self.means.obj.to(objs_normed.device) + self.scales.obj.to(objs_normed.device)*objs_normed if objs_normed is not None else None
